...
Sub new_page(Optional ByVal sheetName As String = "", Optional ByVal pageHead_Row As Integer = 1)
    ...
    With Sheets(sheetName)
        ...
        Call uploadUnsendItems(sheetName)
...
Function setVAL_D() As Boolean
    ...
    valSize = Val(ThisWorkbook.Sheets("值").Cells(1, 3))
...
Sub easyTmpPageDeleter(ByVal id As String)
    ...
    Exit Sub
errorProcess:
    If db.state <> 0 Then Call db.rollback
    row = 1
...
Sub uploadUnsendItems(Optional ByRef except As String = "样本")
    On Error GoTo subend
    Dim result()
    Dim db As New DataBase
    ...
    Dim sql As String
    Dim pageHead As String
    pageHead = getValue("单号头")
    sql = "INSERT INTO `unsend_items_detail`(`collectDate`, `count`, `item`, `pkg`, `qty`, `freight`, `unloadingFee`, `transferFee`, `sum`, `payment`, `comment`, `receverName`, `receverTel`, `senderName`, `senderTel`, `destination`, `pageHead`, `uploadDate`) VALUES "

    Set resultDict = CreateObject("Scripting.Dictionary")
    For Each Sheet In ThisWorkbook.Sheets
        If Sheet.Cells(1, 1) = getValue("清单头") And Sheet.Visible And Sheet.Name <> except Then
            With Sheet
                qtys = .Range(.Cells(5, Int(getValue("件数列"))), .Cells(Int(.Cells(1, Int(getValue("清单长度列")))) - 6, Int(getValue("件数列")))).value
                dates = .Range(.Cells(5, Int(getValue("物流收货日期列"))), .Cells(Int(.Cells(1, Int(getValue("清单长度列")))) - 6, Int(getValue("物流收货日期列")))).value
                destination = .Cells(1, Int(getValue("清单目的地列")))
                For r = 1 To UBound(qtys)
                    If qtys(r, 1) > 0 Then
                        Key = Int(dates(r, 1)) & "|" & destination
                        resultDict(Key) = resultDict(Key) + qtys(r, 1)
                    End If
                Next
                
                datas = .Range(.Cells(5, 1), .Cells(Int(.Cells(1, Int(getValue("清单长度列")))) - 6, Int(getValue("清单宽度"))))
                Debug.Print ""
                For row = 1 To UBound(datas)
                    If datas(row, Int(getValue("件数列"))) > 0 Then
                        sql = sql & "(""" & datas(row, Int(getValue("物流收货日期列"))) & """,""" & datas(row, 1) & """,""" & datas(row, 5) & """,""" & datas(row, 6) & """,""" & datas(row, 7) & """,""" & datas(row, 8) & """,""" & datas(row, 9) & """,""" & datas(row, 10) & """,""" & datas(row, 11) & """,""" & datas(row, 12) & """,""" & datas(row, 13) & """,""" & datas(row, 2) & """,""" & datas(row, 3) & """,""" & datas(row, 14) & """,""" & datas(row, 15) & """,""" & destination & """,""" & pageHead & """,""" & Now & """)," & Chr(10)
                    End If
                Next
            End With
        End If
    ...
    If resultDict.count > 0 Then
        ...
        trans = True
        If Hour(Now) >= 6 Then
            db.execute "DELETE FROM `unsend_items_count` WHERE `uploadDate` >= """ & Date + 0.25 & """ and `uploadDate` < """ & Date + 1.25 & """ and `pageHead` = """ & pageHead & """"
            db.execute "DELETE FROM `unsend_items_detail` WHERE `uploadDate` >= """ & Date + 0.25 & """ and `uploadDate` < """ & Date + 1.25 & """ and `pageHead` = """ & pageHead & """"
        Else
            db.execute "DELETE FROM `unsend_items_count` WHERE `uploadDate` >= """ & Date - 0.75 & """ and `uploadDate` < """ & Date + 0.25 & """ and `pageHead` = """ & pageHead & """"
            db.execute "DELETE FROM `unsend_items_detail` WHERE `uploadDate` >= """ & Date - 0.75 & """ and `uploadDate` < """ & Date + 0.25 & """ and `pageHead` = """ & pageHead & """"
        End If
        db.execute Left(sql, Len(sql) - 2)
        sql = "INSERT INTO `unsend_items_count`(`uploadDate`, `collectDate`, `destination`, `qty`, `pageHead`) VALUES "
        For Each Key In resultDict.keys
            sql = sql & "(""" & Now & """,""" & Split(Key, "|")(0) & """,""" & Split(Key, "|")(1) & """,""" & resultDict(Key) & """,""" & pageHead & """)," & Chr(10)
        Next
        db.execute Left(sql, Len(sql) - 2)
    End If
    db.commit
