...
Sub uploadUnsendItems(Optional ByRef except As String = "样本")
    ...
    Dim sql As String
    sql = "INSERT INTO `unsend_items_detail`(`collectDate`, `count`, `item`, `pkg`, `qty`, `freight`, `unloadingFee`, `transferFee`, `sum`, `payment`, `comment`, `receverName`, `receverTel`, `senderName`, `senderTel`, `destination`) VALUES "
            ...
            With Sheet
                ...
                For row = 1 To UBound(datas)
                    If datas(row, Int(getValue("件数列"))) > 0 Then
                        sql = sql & "(""" & datas(row, Int(getValue("物流收货日期列"))) & """,""" & datas(row, 1) & """,""" & datas(row, 5) & """,""" & datas(row, 6) & """,""" & datas(row, 7) & """,""" & datas(row, 8) & """,""" & datas(row, 9) & """,""" & datas(row, 10) & """,""" & datas(row, 11) & """,""" & datas(row, 12) & """,""" & datas(row, 13) & """,""" & datas(row, 2) & """,""" & datas(row, 3) & """,""" & datas(row, 14) & """,""" & datas(row, 15) & """,""" & destination & """)," & Chr(10)
                    End If
                Next
            End With
        End If
    ...
    If resultDict.count > 0 Then
        ...
        trans = True
        If Hour(Now) >= 6 Then
            db.execute "DELETE FROM `unsend_items_count` WHERE `uploadDate` >= """ & Date + 0.25 & """ and `uploadDate` < """ & Date + 1.25 & """"
        Else
            db.execute "DELETE FROM `unsend_items_count` WHERE `uploadDate` >= """ & Date - 0.75 & """ and `uploadDate` < """ & Date + 0.25 & """"
        End If
        db.execute Left(sql, Len(sql) - 2)
        sql = "INSERT INTO `unsend_items_count`(`uploadDate`, `collectDate`, `destination`, `qty`) VALUES "
        For Each Key In resultDict.keys
            sql = sql & "(""" & Now & """,""" & Split(Key, "|")(0) & """,""" & Split(Key, "|")(1) & """,""" & resultDict(Key) & """)," & Chr(10)
        Next
        db.execute Left(sql, Len(sql) - 2)
    End If
    db.commit
